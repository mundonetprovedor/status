...
export default function App() {
    ...
    switch (status) {
      ...
      case 'Incidente Ativo':
        return {
          ...
          icon: <XCircle className="w-16 h-16 text-rose-400 animate-bounce" />,
      ...
      case 'Manutenção Programada':
        return {
          ...
          icon: <Wrench className="w-16 h-16 text-sky-400" />,
  ...
  const getMaintenanceBadge = (status: string) => {
    ...
    switch (status) {
      ...
      case 'Em Execução':
        return `${base} bg-sky-500/10 border-sky-500/20 text-sky-400 animate-pulse`;
      case 'Concluído':
        return `${base} bg-emerald-500/10 border-emerald-500/20 text-emerald-400`;
      case 'Cancelado':
  ...
  if (loading && !data) {
    return (
      <div className="fixed inset-0 bg-[#060910] flex flex-col items-center justify-center gap-4 z-50">
        ...
        <p className="text-slate-400 animate-pulse font-medium">Carregando status da rede...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#060910] text-slate-200 font-sans pb-12 selection:bg-cyan-500/30">
        ...
        <div className="max-w-5xl mx-auto px-4 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-bold tracking-widest uppercase">Status da Rede</p>
            </div>
          </div>

          <div className="flex items-center gap-4">
            {isRefreshing && (
              <span className="text-xs text-cyan-400/80 animate-pulse flex items-center gap-1.5 font-medium">
                <RefreshCw className="w-3 h-3 animate-spin" /> Atualizando...
              </span>
            )}
            <button 
              onClick={() => fetchStatus(true)}
              className="px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-all text-xs font-bold flex items-center gap-2 hover:border-white/20 active:scale-95"
            >
      ...
      <main className="max-w-4xl mx-auto px-4 mt-8 flex flex-col gap-8">
        {error && (
          <div className="p-4 bg-rose-500/15 border border-rose-500/30 rounded-2xl flex items-start gap-3 text-rose-200 text-sm">
        ...
        <section className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Incidentes Ativos</h3>
            <span className="text-xs bg-white/5 border border-white/10 px-2.5 py-1 rounded-full font-bold">
              {data?.activeIncidents.length || 0} ativo(s)
            </span>
          </div>

          {data && data.activeIncidents.length === 0 ? (
            <div className="p-8 bg-[#0b121f]/30 border border-white/5 rounded-2xl text-center text-slate-400 text-sm font-medium">
            ...
            <div className="flex flex-col gap-4">
              {data?.activeIncidents.map((incident: any) => {
                ...
                return (
                  <div 
                    key={incident.id} 
                    className="border border-white/5 bg-[#0b121f]/40 rounded-2xl overflow-hidden hover:border-white/10 transition-all shadow-lg"
                  >
                    <div 
                    ...
                    >
                      <div className="flex flex-col gap-2 flex-1 min-w-0 pr-4">
                        ...
                        <h4 className="text-base font-bold text-white truncate">{incident.title}</h4>
                      </div>
                    ...
                    {isExpanded && (
                      <div className="px-5 pb-5 pt-1 border-t border-white/5 bg-white/1 flex flex-col gap-4">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          <div className="flex items-start gap-2.5">
                            <MapPin className="w-4 h-4 text-cyan-400 shrink-0 mt-0.5" />
                            <div>
                              <p className="text-[10px] text-slate-400 uppercase font-black tracking-wider">Regiões Afetadas</p>
                              <p className="text-sm font-semibold text-slate-200">{incident.affected_regions}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-start gap-2.5">
                            ...
                            <div>
                              <p className="text-[10px] text-slate-400 uppercase font-black tracking-wider">Clientes Impactados</p>
                              <p className="text-sm font-semibold text-slate-200">{incident.affected_clients}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-start gap-2.5">
                            ...
                            <div>
                              <p className="text-[10px] text-slate-400 uppercase font-black tracking-wider">Horário de Início</p>
                          ...
                          <div className="flex items-start gap-2.5">
                            ...
                            <div>
                              <p className="text-[10px] text-slate-400 uppercase font-black tracking-wider">Previsão de Normalização</p>
                              <p className="text-sm font-semibold text-slate-200">{incident.eta || 'A analisar'}</p>
                            </div>
                          </div>
                        </div>

                        <div className="p-3 bg-white/2 rounded-xl border border-white/5 flex items-center justify-between text-xs text-slate-400">
        ...
        <section className="flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Manutenções Programadas</h3>
            <span className="text-xs bg-white/5 border border-white/10 px-2.5 py-1 rounded-full font-bold">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 border-t border-white/5 pt-4">
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        ...
                        <p className="text-sm font-semibold text-slate-200">{m.schedule_date}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        <p className="text-[10px] text-slate-400 uppercase font-black tracking-wider">Regiões Afetadas</p>
                        <p className="text-sm font-semibold text-slate-200">{m.affected_regions}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        <p className="text-[10px] text-slate-400 uppercase font-black tracking-wider">Impacto Esperado</p>
                        <p className="text-sm font-semibold text-slate-200">{m.expected_impact}</p>
                      </div>
                    </div>
                  </div>
                </div>
        ...
        <section className="flex flex-col gap-4">
          <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest">Histórico Recente (Últimos 30 dias)</h3>
                ...
                >
                  <div className="flex flex-col gap-1 min-w-0 flex-1 pr-4">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="font-bold text-slate-200 truncate">{hist.title}</h4>
                    </div>
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="font-mono font-bold text-white text-xs">{formatDuration(hist.duration_seconds)}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <footer className="max-w-4xl mx-auto px-4 mt-16 text-center border-t border-white/5 pt-8 text-xs text-slate-500 font-medium">
